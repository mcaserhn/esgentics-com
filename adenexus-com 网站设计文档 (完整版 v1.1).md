...
10. [附录 E: 多语言实现方案](#附录-e-多语言实现方案)
11. [交付清单与下一步](#11-交付清单与下一步)
...
```
┌─────────────────────────────────────────┐
│ Logo    Nav Items    [EN ▼]  [Contact] │
└─────────────────────────────────────────┘
```

**交互逻辑**：
1. 点击 "EN ▼" 展开下拉菜单
2. 显示可用语言列表（English, Français, العربية, Deutsch）
3. 选择语言后：
...
<div class="language-switcher" role="navigation" aria-label="Language selection">
  ...
  >
    ...
    <svg class="icon-chevron" width="12" height="12" viewBox="0 0 12 12">
      <path d="M2 4l4 4 4-4" stroke="currentColor" fill="none"/>
    </svg>
  </button>
  
  <ul class="lang-dropdown" role="menu" hidden>
    <li role="none">
      <a 
        href="/en" 
        data-lang="en" 
...
.lang-dropdown {
  ...
  border: 1px solid var(--medium-gray);
  border-radius: 4px;
  box-shadow: 0 4px 16px rgba(40,57,72,0.15);
  list-style: none;
  padding: 8px 0;
  margin: 0;
  z-index: 1000;
...
class LanguageSwitcher {
  ...
  init() {
    ...
    this.items.forEach(item => {
      item.addEventListener('click', (e) => this.selectLanguage(e));
    });
  ...
  setActiveLanguage(lang) {
    // 更新按钮文字
    this.toggle.querySelector('.lang-current').textContent = lang.toUpperCase();
...
|---------|------|---------|--------|
| v1.2 | 2026-02-26 | Added comprehensive multilingual implementation plan (Appendix E): language strategy, URL routing, language switcher component, translation management, SEO configuration, RTL support, implementation timeline, cost estimation | ESGentics Team |
...
**文档结束** | Version 1.2 Complete | ESGentics Pte. Ltd. | Last Updated: 2026-02-26
...
主要更新内容：
1. ✅ 在目录中添加了"附录 E: 多语言实现方案"
2. ✅ 更新了 1.2 关键约束中的语言要求
3. ✅ 新增了完整的附录 E（包含 11 个详细章节）
4. ✅ 更新了版本历史（v1.2）

**附录 E 包含**：
- 语言支持策略与优先级
- URL 路径前缀路由方案
- 语言切换器完整代码（HTML/CSS/JS）
- 内容翻译管理流程（JSON 结构 + 工具推荐）
- SEO 多语言配置（hreflang + sitemap）
- RTL（阿拉伯语）布局支持
- 开发实施步骤与时间线
- 多语言 QA 验收清单
- 成本估算（分阶段）
