        ...
        <div className="container-content">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {translations?.about?.mission?.beliefs?.map((belief, index) => (
              <motion.div 
                key={index}
                className="bg-aden-light-gray rounded-lg p-8 shadow-sm"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.6, delay: index * 0.1 }}
              >
                <p className="text-aden-dark-gray font-body">{belief}</p>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="container-content">
          <div className="flex flex-col md:flex-row items-center justify-between gap-10">
            <motion.div 
              ...
              transition={{ duration: 0.6 }}
            >
              <h2 className="text-3xl md:text-4xl font-title font-bold mb-6">{translations?.about?.technology_partner?.title || 'Our Technology Partner'}</h2>
              ...
              <div className="space-y-4">
                {translations?.about?.technology_partner?.features?.map((feature, index) => (
                  <div key={index} className="flex items-start">
                    ...
                    <p className="text-aden-dark-gray font-body">{feature}</p>
                  </div>
                ))}
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="w-full h-64 md:h-80 bg-gray-100 rounded-lg flex items-center justify-center">
                <div className="text-center">
                  <div className="w-32 h-32 mx-auto mb-4 border-4 border-aden-orange rounded-full flex items-center justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-aden-orange">
                  ...
                  <p className="text-aden-dark-gray font-title font-bold">NVIDIA Partner</p>
                </div>
              </div>
            </motion.div>
          </div>
      ...
      <section className="py-20 bg-aden-light-gray">
        <div className="container-content">
          <motion.div 
            ...
            transition={{ duration: 0.6 }}
          >
            <h2 className="text-3xl md:text-4xl font-title font-bold mb-6">{translations?.about?.ai_platform?.title || 'The AI Platform for Sustainable Buildings'}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {translations?.about?.ai_platform?.features?.map((feature, index) => (
              <motion.div 
                ...
                transition={{ duration: 0.6, delay: index * 0.1 }}
              >
                <p className="text-aden-dark-gray font-body font-medium">{feature}</p>
              </motion.div>
            ))}
          </div>
          <motion.p 
            className="text-center text-lg text-aden-dark-gray mt-12 font-body"
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
